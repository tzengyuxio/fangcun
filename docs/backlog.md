...
13. 🔜 **（高優先，排在 CN／TW／HK notes 改寫完成後做）CN 郵票照片改用 WNS 圖源** —
    現有 CN 圖下載自 5151sc，**帶浮水印**；改用無浮水印、品質一致的單枚 WNS 圖（`images/T180/{WNS}.jpg`）。
    - **作法**:用 `wns-query` 取各枚 WNS 號 → 下載對應單枚圖 → 對應到各 `items[]`；順帶填 `items[].wns`。
    - **拆圖**:5151sc 部分圖是「一張圖含兩枚郵票」，需裁切拆開分別對應到兩個 `items[]`。
    - **⚠ 覆蓋風險（已查證）**:**WNS-China 提交極不全**——實測僅 **2023** 有資料（CN001/CN002.2023），
      2016/2018/2020/2024/2025/2026 等多年 WNS 回 **0 筆**。故多數 CN 年份**無 WNS 圖可換**，本任務
      對多數年份可能受阻;需先確認各年 WNS 覆蓋，無 WNS 者另尋無浮水印來源（官方／自掃）或保留現況。
    - **注意**:WNS 圖只含郵票本體、不含小全張;著作權屬發行郵政，須標註「© 中國郵政 via UPU/WADP WNS」。
    - 依賴 `wns-query` skill;與 item 9/10（港日 WNS）作法相通。
