...
8. **圖版控**：`public/img/stamps/<cc>/` 預設被 `public/img/stamps/*` 忽略，要 `!` 放行該區。
   替代圖放 `_alt/`（gitignore 已排除）、官方圖默認 commit;**不需 official/ 目錄**（官方圖是默認狀態，
   只有替代圖需標記）;**verified 與圖脫鉤**（資料查好就 true，圖臨時性靠 `_alt/`/fallback 處理）。

## 踩坑清單（日本實戰）

9. **WNS ≠ 該國生肖票全集**：日本お年玉付年賀切手（主力生肖票）**不提交 WNS**，WNS 只收「干支文字
   greeting」。先 member 全量掃確認 WNS 實際收了什麼、並**對照現有 catalog**——可能補強現有、也可能
   新建另一系列;**別假設 WNS = 該國生肖票全集、別假設 1:1**。日本實跑主軸因此從「補強」變「新建干支文字系列」。
10. **建檔前 `git status` 查覆蓋**：腳本寫 `<cc>-<date>.json`，若與現有同日套撞會**直接覆蓋**（日本年賀
    主票＋干支文字竟同日發行!）。建檔後**先看 `git status` 有沒有 `M`**（覆蓋既有），撞了按 id-scheme
    §2.2 拆 `-a`（主票/主生肖票）/`-b`（其餘）。
11. **每年多枚＋截斷**：量大郵政（日本一年 300–400+ 枚）`--page-size` 要夠（400+）;`count` 滿頁=截斷，
    `--page 1` 看真總數。生肖票常只是其中一小撮（日本每年固定 10 枚干支文字）。
12. **denom 格式各國不一**：日本 denom 含單位「80 JPY」（cur 空）、美國「Forever」/「0.34」;建檔腳本
    逐國解析（`.split()[0]` 取數字），currency 另給。
13. **官方頁面補 WNS 缺、且是第二 official**：WNS 常缺 designer/mintage/印法/書家;該國郵政官方發行頁有，
    但 URL 逐套不規則（日本 `h{平成年}{月日}_t.html`），逐年查。**雙 official（WNS＋官方頁）建檔品質最高**。
14. **D8 邊界（文字/問候票）**：生肖文字書法、greeting 切手算不算生肖票，是邊界案例，**站主拍板**
    （日本干支文字已拍板收）。先查清該系列性質（官方頁／集郵資料）再判。
15. **D5 發行模式各國不同**：日本**年底發行→次年生肖（全部 offset）**、美國多當年初發（少數提前年底）、
    台灣提前一年;`zodiac_year` 逐套依生肖、`issue_date` 依實際發行，永不混用。
